...
> ANSWER: Narrow data is presented with one column containing all the values. In this case, the variable "name" is that column. Narrow data has other columns listing the context of the value, which are the variables "sex" and "total" in BabyNarrow. A wide dataset like babynames2 has each observation as an individual with each different data variable in a separate column. 
...
> ANSWER: The number of observations is different because we had previously filtered for names where both `M` and `F` are greater than 10,000. Thus the total number of names has decreased dramatically. We have twice as many observations as BabyWide because we combined 'male' and 'female' into one column. 
