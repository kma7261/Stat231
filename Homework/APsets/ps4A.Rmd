...
> ANSWER: There are twelve tables, which appears to be the same to the online textbook for whatever reason.

```{r}
#Copy and pasted from the textbook
...
> ANSWER: The world record for the indoor one-mile world record for IAAF women is Genzebe Dibaba with a time of 4 minutes and 13.31 seconds.

```{r}
#Pluck table 10, which gives the wanted table
women_indoor <- tables %>%
  purrr::pluck(10) %>%
  html_table()
women_indoor
...
> ANSWER: The fastest record is from an outdoor event run by Sifan Hassan with a time of 4:12.33.

```{r}
#pluck table number 8, which is the outdoor IAAF stuff
#Mutate a column "type" for indoor and outdoor that describes whether it is indoor or outdoor
...
women_indoor <- women_indoor %>%
  mutate(Type = "indoor")
#Bind datasets together, arrange by time
women <- women_outdoor %>%
  bind_rows(women_indoor) %>%
  arrange(Time)
women
```
