...
> ANSWER: The `str_subset()` function uses a needle in a haystack paradigm. In this case, it returns all of the lines that start with " MACBETH" which are Macbeth's speaking lines. The `str_which()` function returns the indices within the macbeth vector of where the word " MACBETH" shows up. Lastly, `str_detect` works by returning true or false for all of the indices on whether the word " MACBETH" matches. 
...
> ANSWER: Here we introduce a metacharacter, the period. The first line `head(str_subset(macbeth, "MACBETH\\."))` allows us to search for the actual character string that has the period at the end. The second line code includes the lines that allow sentences that have "MACBETH" and a comma to stay. The reason is the two `\\` allows us to denote the literal value of the period, otherwise it matches with any character, including commas.
...
> ANSWER: In the first case, the pattern "MAC[B-Z]" indicates that any line that contains "MAC" where the "MAC" is followed by any capital letter other than "A" will show up. For the second line, "MAC[B|Z]" it matches any pattern that has "MAC" followed by a capital "B" or a capital "Z". For the third line, "^MAC[B-Z]", the patter indicates that we will search for a pattern that only matches with lines that START with "MAC" and are followed by any letter other than the capital "A".
