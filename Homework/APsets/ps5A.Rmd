...
> ANSWER: The `str_subset()` function uses a needle in a haystack paradigm. In this case, it returns all of the lines that start with " MACBETH" which are Macbeth's speaking lines. The `str_which()` function returns the indices of where the word " MACBETH" shows up. Lastly, `str_detect` works by returning true or false for all of the indices on whether the word " MACBETH" matches. 
