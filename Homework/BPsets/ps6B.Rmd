...
# Remove stop words and unite back
...
iPhone <- bigrams_filtered %>%
  filter(source == "iPhone") %>%
  arrange(desc(n)) 
iPhone_ten <- iPhone[1:10,]
#combine
trump_proc <- rbind(android_ten, iPhone_ten)

trump_proc %>% 
  ggplot(aes(bigram, n, fill = source)) +
