...
> RESPONSE: 1846 rows and 3 columns. The variables are dataset: indicates which dataset the data are from, x: x-values, and y:y-values
...
> RESPONSE: The correlation coefficient between 'x' and 'y' in the 'dino' dataset is -0.6447185. This correlation coefficient implies that 'x' and 'y' in the 'dino' dataset are negatively correlated. As such, which 'x' increases, 'y' tends to decrease.
...
> RESPONSE: First, the x and y coordinates make up dots that form the shape of a dinosaur. Overall it seems that as x increases y increases or decreases.
...
> RESPONSE: The correlation between x and y in the 'star' dataset is -0.0629611. This correlation is slightly less negative than the correlation between x and y in the 'dino' dataset. 
...
> RESPONSE: The plot does not look the same. In this plot the dots make up the shape of a star instead of a dinosaur.
...
> RESPONSE: The correlation between 'x' and 'y' in the circle dataset is -0.06834336. This value is more negative than both of the correlations of the previous two datasets. 
...
> RESPONSE: The plot does not look like the same as the other two plots. Instead, the dots form the shape of a circle.
...
> RESPONSE: The correlation coefficients for all of the datasets are relatively similar, between 0.06 and 0.07. However, all of the datasets create different shapes, which means that the correlation coefficient is not an appropriate summary for these datasets. Furthermore, these datasets have distinct patterns which means that the correlation coefficient really doesn't tell us anything about any of the datasets.
