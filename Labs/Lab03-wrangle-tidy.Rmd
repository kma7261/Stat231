...
```{r}
#Load CSV files
coverage <- read_csv("/Users/kevinma/Github/Stat231/healthcare_coverage.txt")
spending <- read_csv("/Users/kevinma/Github/Stat231/healthcare_spending.txt")
```
...
> RESPONSE: The spending dataset covers years from 1991 to 2014. The coverage dataset covers years from 2013 to 2016.

```{r}
#Look at spending and coverage
glimpse(spending)
glimpse(coverage)

```
```{r}
#Narrow down to columns that we want, which are years
spending <- spending[, -c(2:23)]
coverage <- coverage[,-c(15:29)]
glimpse(coverage)
```
...
```{r}
#Filter and remove observations
spending <- spending %>%
...
> RESPONSE: Each observation should have state, year, and amount of spending.

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: We need to combine the datasets so that they are in one dataset. Next, we also need to join the columns of 2013 and 2014 employer coverage into one column
...
```{r}
#Join and gather data
coverage <- coverage %>%
  select(`2013__Employer`,`2014__Employer`,Location)
coverage_2013 <- coverage %>%
  select(`2013__Employer`,Location) %>%
  rename(coverage = "2013__Employer")
coverage_2014 <- coverage %>%
  select(`2014__Employer`,Location) %>%
  rename(coverage = "2014__Employer")
spending_2013 <- spending %>%
  select(`2013__Total Health Spending`,Location) %>%
  rename(spending = "2013__Total Health Spending") 
spending_2014 <- spending %>%
  select(`2014__Total Health Spending`,Location) %>%
  rename(spending = "2014__Total Health Spending")

#Bind coverage and spending datasets by year
dataframe_2013 <- coverage_2013 %>% 
  inner_join(spending_2013, by = "Location") %>% 
  mutate(year = "2013")
dataframe_2014 <- coverage_2014 %>% 
  inner_join(spending_2014, by = "Location") %>% 
  mutate(year = "2014")

#stack on top of each other
final_df <- rbind.data.frame(dataframe_2013, dataframe_2014)

ggplot(data = final_df, mapping = aes(x = spending))
```
