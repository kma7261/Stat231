...
tables <- url %>%
  read_html() %>%
  html_nodes("table")
dickinson_poem <- html_table(tables[[1]])
dickinson_poem
```
...
```{r}
url_table <- dickinson_poem %>%
  mutate(url = paste("https://en.wikisource.org/wiki/", str_replace_all(`First Line (often used as title)`, " ", "_"), sep = "")) 
```
...
all_poems <- data.frame(name = url_table$`First Line (often used as title)`
                        , text = ""
