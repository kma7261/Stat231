...
```{r}
coverage <- read_csv("/Users/claraseo/Dropbox (Amherst College)/2021 SPRING/STAT 231/course-content/data/healthcare_coverage.txt")
# coverage <- read_csv("/Users/claraseo/Dropbox (Amherst College)/2021 SPRING/STAT 231/clara-stat231-spring21/data/healthcare_coverage.txt")
spending <- read_csv("/Users/claraseo/Dropbox (Amherst College)/2021 SPRING/STAT 231/course-content/data/healthcare_spending.txt")
```
...
> RESPONSE: In the `spending` dataset, years 1991 to 2014 are covered. In the `coverage` dataset, years 2013 to 2016 are covered. 

```{r}
library(mosaic)

# determining years covered for each dataset
names(spending)
names(coverage)

# select data from years for which we have BOTH spending and coverage info
spending <- spending %>% 
...
```{r}
spending <- spending %>% 
  filter(!Location %in% c("United States", "District of Columbia"))
...
> RESPONSE: The variables I need to create the scatterplot in ggplot: `spending`, `employer_coverage`, and `year`. Each observation will represent the spending and employer coverage of a specific year for a particular state.

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: I will add a variable called `year` to the `spending` and `coverage` data sets. Then I will join the `spending` and `coverage` data sets by the states. 
...
```{r}
# subset spending data set by year 
spending2013 <- spending %>% 
  select(Location, "2013__Total Health Spending") %>%
  rename(spending = "2013__Total Health Spending")
spending2014 <- spending %>% 
  select(Location, "2014__Total Health Spending") %>%
  rename(spending = "2014__Total Health Spending")

# subset coverage data set by year 
coverage2013 <- coverage %>%
  select(Location, "2013__Employer") %>%
  rename(employer_coverage = "2013__Employer")
coverage2014 <- coverage %>%
  select(Location, "2014__Employer") %>%
  rename(employer_coverage = "2014__Employer")

# combine spending and coverage data sets by year
year2013 <- spending2013 %>% 
  inner_join(coverage2013, by="Location") %>%
  mutate(year = "2013")
year2014 <- spending2014 %>% 
  inner_join(coverage2014, by="Location") %>%
  mutate(year = "2014")

# combine the data from both years into a single data set 
df <- rbind.data.frame(year2013, year2014) %>% 
  clean_names()

ggplot(df, aes(x = log(spending), y = log(employer_coverage), color = year)) +
  geom_point() + 
  labs(x = "log(Spending)", y = "log(Employer Coverage)")
```
...
# total population data for every state by year 
total_pop_2013 <- coverage %>% 
  select(Location, "2013__Total") %>%
  rename(total_pop = "2013__Total")
total_pop_2014 <- coverage %>% 
  select(Location, "2014__Total")%>%
  rename(total_pop = "2014__Total")

# added year-specific total population info to data sets with spending/coverage info 
year2013 <- year2013 %>% 
  inner_join(total_pop_2013, by = "Location")
year2014 <- year2014 %>% 
  inner_join(total_pop_2014, by = "Location")

# combine 2013 and 2014 info into one data set 
df <- rbind.data.frame(year2013, year2014) %>% 
  janitor::clean_names()

ggplot(data = df, aes(x = total_pop, y = employer_coverage, color = year)) + 
  geom_point() + 
  labs(x = "Total Population (count)", y = "Employer Coverage")

ggplot(data = df, aes(x = total_pop, y = spending, color = year)) + 
  geom_point() + 
  labs(x = "Total Population (count)", y = "Spending")
```

Lastly, to account for total population, create a scatterplot of spending per capita versus proportion with employer coverage.  This time, *color by region* and *facet by year*.  The total spending column is reported in millions (1e6). Therefore, to calculate `spending_capita` we will need to adjust for this scaling factor to report it on the original scale (just dollars) and then divide by `total_pop`.  Based on this figure, write a brief paragraph describing the relationship between health care spending and coverage in the US.

> RESPONSE: There appears to be a positive, moderate, and linear association between health care spending and coverage in the US. In terms of regional trends, it appears that the states in the North and North Central tend to have larger values for both spending per capita and proportion with empolyer coverage, whereas the states in the South and West tend to have smaller values for both variables. 

```{r}
# combined state_info and df data sets to add information about region
df <- state_info %>%
  clean_names() %>% 
  inner_join(df, by = "location")

df <- df %>% 
  mutate(spending_capita = (spending*1e6)/total_pop,
         prop_employer_coverage = employer_coverage/total_pop) %>%
  clean_names()

ggplot(data = df, aes(x = spending_capita, y = prop_employer_coverage, color = region)) + 
  facet_wrap(~year) + 
  geom_point() + 
  labs(x = "Spending Per Capita (dollars)", y = "Proportion with Employer Coverage")
```
...
Re-create this plot for 2013: ![Uninsured vs. Spending in 2013](/Users/claraseo/Dropbox (Amherst College)/2021 SPRING/STAT 231/course-content/images/uninsured_spending_2013.png)
