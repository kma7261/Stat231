...
> ANSWER: 107 nodes, 352 edges

```{r}
# create as network object
# identify number of nodes
got_net <- graph_from_data_frame(got, directed = FALSE)
vcount(got_net)
...
> ANSWER: 352 edges. The density is 0.062 which means that it is not that dense.

```{r}
# note that the number of *possible* edges in an undirected graph is (V choose 2) = V*(V-1)/2
ecount(got_net)
x = vcount(got_net)
prop = ecount(got_net)/(x*(x-1)/2)
prop
```
...
> ANSWER: It means that there is a path from every node to any other node either directly or through paths from other nodes. You can go from node A to node B through the network.

```{r}
is.connected(got_net)
```


4.  Use the code below to compute the diameter of the network.  Intepret the value.

> ANSWER: Diameter of the network is 6. The shortest distance between the two most distant nodes is 6.

```{r}
diameter(got_net, directed = FALSE)
```
...
```{r}
V(got_net)$degree <- igraph::degree(got_net)
V(got_net)$wtdegree <- strength(got_net
                                , weights=E(got_net)$Weight)

stats <- data_frame(name = V(got_net)$name, 
                    degree = V(got_net)$degree, 
                    wtdegree = V(got_net)$wtdegree )
```
...
> ANSWER: The five characters with the highest degreee are Tyrion, Jon, Sansa, Robb, and Jaime. The five characters with the highest weighted degreee are Tyrion, Jon, Sansa, Jaime, and Bran.
...
> ANSWER: Robb can have a higher degree than Bran if there are more edges that are connected to Robb than Bran. However, if the edges from Bran have a higher weight it is possible for Bran to have a higher weighted degree.
...
```{r}
V(got_net)$betweenness <- igraph::betweenness(got_net, weights=NA)
stats$betweenness <- V(got_net)$betweenness
stats3 <- stats %>%
...
```{r}
stats <- stats %>%
   mutate(eigen = eigen_centrality(got_net, directed = FALSE
                          , weights = NA)$vector
         , page.rank = page_rank(got_net, directed = FALSE
                          , weights = NA)$vector
  )
```
